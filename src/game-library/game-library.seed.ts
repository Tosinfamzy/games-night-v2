...
/**
 * The shared game-library catalog, seeded server-side on module init
 * (see GameLibraryService.seedInitialGames) and idempotently by name.
 *
 * Planning metadata (`format`, `recommendedRounds`, `playersPerRound`,
 * `minTeams`, `winnerBonusPoints`) powers the Night Builder's suggestions and
 * mirrors how these games were actually run at the launch event. Games that
 * omit a field take the entity default (format `all_teams`, 1 round, everyone
 * plays, no special team requirement, placement points).
 *
 * Kept as data separate from the service so the values are easy to review and
 * unit-test.
 */
export const INITIAL_GAMES: CreateGameLibraryDto[] = [
  {
    name: 'Articulate',
    description:
