...
export class TeamFormationService {
  ...
  /**
   * Create teams for a game with automatic player assignment
   */
  ...
  /**
   * Assign players to teams based on a given strategy
   * Used by both this service and TeamAssignmentService
   */
  async assignPlayersByStrategy(
    teams: Team[],
    players: Player[],
    strategy: TeamFormationStrategy,
  ): Promise<void> {
  ...
  validateTeamFormation(
    ...
    if (teamCount > LIMITS.MAX_TEAMS_PER_GAME) {
      errors.push(`Maximum ${LIMITS.MAX_TEAMS_PER_GAME} teams allowed`);
    }
